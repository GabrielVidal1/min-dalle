...
def sanitize_prompt(text) :
  ...
  return "".join([c for c in text.replace(' ', '_') if c in valid_chars])
        ...
        for row_index in range(row_count):
            ...
            if ((row_index + 1) * (2 ** log2_mid_count)) % row_count == 0:
                ...
                if row_index + 1 == row_count and (save_individual_images or save_grid_image) :
                    ...
                    file_name = f"{seed:05d}_" + time.strftime("%Y%m%d-%H%M%S")
                    if save_individual_images :
                        self.save_images(images, save_dir, file_name)
                    if save_grid_image :
                        image_grid.save(os.path.join(save_dir, f"{file_name}_grid.png"))
